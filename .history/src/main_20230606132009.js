...
function pintarPosters(listaDePeliculas) { //estoy pintando los posters para eso es la función
  for (let index = 0; index < listaDePeliculas.length; index++) {
    ...
    image.classList.add("soloPosters");  //vinculando con las clases, ejemplo class list //add para añadir
    document.getElementById("animations").appendChild(image);//appendChild hace la magia "interaccion con el DOM"
  }
  return
